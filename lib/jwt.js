...
const issuer = 'https://www.upschol.com/'
const audience = 'https://www.upschol.com/'

export const signJWT = async (payload) => {
	const privateKey = await jose.importJWK(pvt_jwk, alg)
...
export const verifyJWT = async (data) => {
	const publicKey = await jose.importJWK(public_jwk, alg);
	const { payload, protectedHeader } = await jose.jwtVerify(data.token, publicKey, {
		issuer: issuer,
		audience: audience,
	});
	return payload;
